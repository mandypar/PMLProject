...
modelFit;
```
So the accuracy of the training dataset comes out at 99.17%  It will be interesting to compare that with the test dataset when processed.
...
So the accuracy of this prediction model is 99.38% when we process the test dataset and the out of sample error is 0.61%.  So that means that the predictions improved slightly (0.21%) when processing the test as opposed to the training dataset.
